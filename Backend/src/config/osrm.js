 ...
 * Car and foot profiles point to the Dockerized OSRM services by default.
 * Bike remains configurable through env because the stack does not ship a bike container.
 * OSRM is the routing engine — it computes actual turn-by-turn road routes
 * using pre-processed OpenStreetMap data.
 */
const osrmConfig = {
  car: {
    url: process.env.OSRM_CAR_URL || 'http://nav_osrm_car:5000',
    profile: 'car',
    // Car-specific options: avoids pedestrian ways, respects one-ways, etc.
  },
  foot: {
    url: process.env.OSRM_FOOT_URL || 'http://nav_osrm_foot:5000',
    profile: 'foot',
