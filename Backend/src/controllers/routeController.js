...
/**
 * GET /api/route/stops-in-bbox?minLng=&minLat=&maxLng=&maxLat=&limit=300
 * Return all transit stops within a map viewport bounding box.
 * Used by the frontend map layer to display stops as the user pans/zooms.
 */
async function getStopsInBbox(req, res, next) {
  try {
    const { minLng, minLat, maxLng, maxLat, limit = 300 } = req.query;
    if (!minLng || !minLat || !maxLng || !maxLat) {
      return res.status(400).json({
        success: false,
        error: 'Required: minLng, minLat, maxLng, maxLat',
      });
    }
