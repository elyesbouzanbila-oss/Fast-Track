...
async function updateHazard(req, res, next) {
  try {
    ...
    const allowed = ['type', 'severity', 'description', 'radius_meters', 'expires_at', 'is_active'];
    const updates = {};
    allowed.forEach((field) => {
      if (req.body[field] !== undefined) updates[field] = req.body[field];
    });

    updates.expires_at = resolveHazardExpiry(
      updates.type !== undefined ? updates.type : hazard.type,
      updates.severity !== undefined ? updates.severity : hazard.severity,
      updates.expires_at !== undefined ? updates.expires_at : hazard.expires_at
    );
